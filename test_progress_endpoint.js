...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJFUzI1NiIsImtpZCI6ImI4MTI2OWYxLTIxZDgtNGYyZS1iNzE5LWMyMjQwYTg0MGQ5MCIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjIwODQ4MjIyNTd9.7LVOG62shhOz6jaYkziS5CmsQOFL078CruO8iZ6Vs6yGlquhVppzgIOcHyq9zRXykOvJm6YhG1AENOt81_G6MA';
...
async function getVocabulary() {
  ...
  const response = await fetch(`${SUPABASE_URL}/rest/v1/vocabulary?language_code=eq.pl&category=eq.zwierzeta&order=word_text.asc&limit=10`, {
    headers: {
      'apikey': SUPABASE_ANON_KEY,
      'Authorization': `Bearer ${accessToken}`
    }
...
async function verifyDatabase() {
  ...
  if (progressRecords.length > 0) {
    ...
    progressRecords.slice(0, 5).forEach((record, index) => {
      console.log(`   ${index + 1}. Stars: ${record.stars_earned}, Mastered: ${record.is_mastered}, Attempts: ${record.attempts_count}`);
    });
  }
